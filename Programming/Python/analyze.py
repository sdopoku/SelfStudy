...
HEADER = [ "tweet_id","in_reply_to_status_id","in_reply_to_user_id","timestamp","source","text","retweeted_status_id", \
    ...
    for f in tweet_files:
        ...
        with open(file_path,'r') as csvfile:
            ...
            for row in csvreader:
                tweet = {column:part for column, part in zip(header,row)}
                tweets.append(tweet)
...
def get_word_clusters(tweets):
    ...
    cluster.cluster([vectorspaced( tweet['text'], all_words) for tweet in tweets])

    classified_examples = [
        cluster.classify(vectorspaced( tweet['text'], all_words)) for tweet in tweets
    ]
